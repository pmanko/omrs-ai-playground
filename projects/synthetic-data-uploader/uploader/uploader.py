...
class Uploader:
  ...
  def _extract_openmrs_identifier_value(self, patient_json: Dict[str, str]) -> str:
    """Returns the OpenMRS ID value from the prepared Patient JSON if present."""
    for identifier in patient_json.get('identifier', []):
      id_type = identifier.get('type', {})
      if id_type.get('text') == 'OpenMRS ID':
        return identifier.get('value')
    return ''

  def _upload_patient_with_conflict_resolution(self, patient_json: Dict[str, str]) -> str:
    """Uploads Patient; if identifier conflict, fetch existing and continue.

    Returns the new or existing Patient id, or empty string if unrecoverable.
    """
    try:
      return self._upload_resource('Patient', patient_json)
    except ValueError as exc:
      msg = str(exc)
      if 'Identifier' in msg and 'already in use' in msg:
        identifier_value = self._extract_openmrs_identifier_value(patient_json)
        if identifier_value:
          existing_id = self.fhir_client.find_patient_id_by_identifier(identifier_value)
          if existing_id:
            self.logger.info('Reusing existing Patient id %s for identifier %s', existing_id, identifier_value)
            return existing_id
      # Unrecoverable
      self.logger.error('Patient upload failed and could not recover: %s', msg)
      return ''
  ...
  def upload_openmrs_bundle(self, json_file: pathlib.Path,
    ...
    try:
      ...
      patient_id = self._upload_patient_with_conflict_resolution(
          each_bundle.openmrs_patient.base.json)
      if not patient_id:
        # Skip this bundle; continue with next file
        return
      each_bundle.openmrs_patient.base.new_id = patient_id
      ...
      for openmrs_encounter in each_bundle.openmrs_encounters:
        try:
          ...
          self.logger.error('Encounter upload failed, skipping: %s', str(exc))
          continue
      ...
      for openmrs_observation in each_bundle.openmrs_observations:
        try:
          ...
          self.logger.error('Observation upload failed, skipping: %s', str(exc))
          continue
